...
const DynamicBorrowContent = dynamic(
  () => Promise.resolve(BorrowContentInner),
  {
    ssr: false,
  },
);
...
function BorrowContentInner() {
  ...
  const handleBorrow = async () => {
    ...
    const borrowAmountBigInt = BigInt(
      Math.floor(Number.parseFloat(amount) * 1e18),
    );
    ...
    try {
      await borrow(selectedAsset, amount, selectedDestChain.toString());
          ...
          {address && (
            <>
                ...
                <div className="bg-gray-800 rounded p-4 space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span
                        className={`font-medium ${getHealthFactorColor(healthStatus)}`}
                      >
                        {formatHealthFactor(
                          Number(position.healthFactor) / 1e18,
                        )}{" "}
                        ({healthStatus})
                      </span>
                    </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-gray-400 text-sm mb-1 block">
                    Destination Chain (Receive Assets)
                  </label>
                  <select
              ...
              <div className="bg-gray-800 rounded p-3 text-xs space-y-1">
                <div className="text-gray-400 font-medium">
                  Cross-Chain Transaction Preview:
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  <span>
                    {
                      CCIP_CONFIG[
                        selectedSourceChain as keyof typeof CCIP_CONFIG
                      ]?.name
                    }
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span>
                    {
                      CCIP_CONFIG[selectedDestChain as keyof typeof CCIP_CONFIG]
                        ?.name
                    }
                  </span>
                </div>
              ...
              {amount && Number.parseFloat(amount) > 0 && (
                <div className="bg-yellow-900/20 border border-yellow-600/30 rounded p-3 text-xs">
                  <div className="text-yellow-400 font-medium">
                    ⚠️ Health Factor Impact
                  </div>
                  <div className="text-gray-300 mt-1">
                    Borrowing will decrease your health factor. Ensure it stays
                    above 1.0 to avoid liquidation.
                  </div>
                </div>
              )}
