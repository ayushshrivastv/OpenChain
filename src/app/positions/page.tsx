...
import { CHAINLINK_PRICE_FEED_ABI, formatUnits } from "@/lib/contracts";
...
const DynamicPositionsContent = dynamic(
  () => Promise.resolve(PositionsContentInner),
  {
    ssr: false,
  },
);
          ...
          try {
            ...
            if (collateralPrice && borrowPrice) {
              const collateralValue =
                Number(formatUnits(position.collateralAmount, 18)) *
                Number(formatUnits(collateralPrice[0], 8));
              const borrowValue =
                Number(formatUnits(position.borrowAmount, 18)) *
                Number(formatUnits(borrowPrice[0], 8));
      ...
      <div className="w-full max-w-6xl space-y-6">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-400">
            Monitor your lending and borrowing positions across chains
          </p>
        </div>
                  ...
                  <div className="text-2xl font-bold">
                    ...
                    {positions
                      .reduce(
                        (sum, pos) =>
                          sum +
                          Number(formatUnits(pos.collateralAmount, 18)) * 1000,
                        0,
                      )
                      .toFixed(2)}
                  </div>
                  ...
                  <div className="text-2xl font-bold">
                    ...
                    {positions
                      .reduce(
                        (sum, pos) =>
                          sum +
                          Number(formatUnits(pos.borrowAmount, 18)) * 1000,
                        0,
                      )
                      .toFixed(2)}
                  </div>
                  <p className="text-sm text-gray-400">
                    Available to borrow more
                  </p>
                </CardContent>
              </Card>
                    ...
                    {positions.length > 0
                      ? (
                          positions.reduce(
                            (sum, pos) =>
                              sum +
                              (pos.healthFactor === Number.POSITIVE_INFINITY
                                ? 0
                                : pos.healthFactor),
                            0,
                          ) /
                      ...
                      <tbody>
                        {positions.map((position) => (
                          ...
                          >
                            ...
                            <td className="py-3 px-4">
                              {Number(
                                formatUnits(position.borrowAmount, 18),
                              ).toFixed(4)}
                            </td>
                            <td
                              className={`py-3 px-4 font-medium ${getHealthFactorColor(position.healthFactor)}`}
                            >
                            ...
                            <td className="py-3 px-4">
                              <div className="flex gap-2">
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() =>
                                    openModal("deposit", position.asset)
                                  }
                                >
                                  Supply
                                </Button>
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() =>
                                    openModal("borrow", position.asset)
                                  }
                                >
                                  Borrow
                                </Button>
                                {position.borrowAmount > 0n && (
                                  <Button
                                    ...
                                    variant="outline"
                                    onClick={() =>
                                      openModal("repay", position.asset)
                                    }
                                  >
                                    Repay
                                  </Button>
                                ...
                                {position.collateralAmount > 0n && (
                                  <Button
                                    ...
                                    variant="outline"
                                    onClick={() =>
                                      openModal("withdraw", position.asset)
                                    }
                                  >
                                    Withdraw
                                  </Button>
