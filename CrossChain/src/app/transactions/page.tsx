...
const DynamicTransactionsContent = dynamic(() => Promise.resolve(TransactionsContentInner), {
  ssr: false,
})
...
function StatusIndicator({ status }: { status: string }) {
  const color = status === 'completed' ? 'bg-green-500' : status === 'pending' ? 'bg-yellow-400' : 'bg-red-400'
        ...
        {address && (
          <>
            ...
            {pendingTransactions.length > 0 && (
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 text-yellow-400">Pending Transactions</h3>
                    ...
                    <tbody>
                      {pendingTransactions.map((tx) => (
                        <tr key={tx.id} className="border-b border-gray-800 text-white">
                          ...
                          <td className="py-2 px-3">{formatUnits(tx.amount, 18)} {tx.asset}</td>
