import { Card } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';

export default function BorrowPage() {
  return (
    <div className="w-full flex flex-col items-center justify-center">
      <Card className="w-full max-w-md bg-gray-900 border border-gray-800 p-8 flex flex-col gap-6">
        <div className="text-xl font-semibold mb-2">Borrow</div>
        <div className="flex flex-col gap-4">
          <div>
            <div className="text-gray-400 text-sm mb-1">Destination Chain</div>
            <select className="bg-black border border-gray-800 rounded px-3 py-2 w-full">
              <option>Ethereum Sepolia</option>
              <option>Polygon Mumbai</option>
              <option>Solana Devnet</option>
            </select>
          </div>
          <div>
            <div className="text-gray-400 text-sm mb-1">Asset</div>
            <select className="bg-black border border-gray-800 rounded px-3 py-2 w-full">
              <option>USDC</option>
              <option>WETH</option>
              <option>SOL</option>
            </select>
          </div>
          <div>
            <div className="text-gray-400 text-sm mb-1">Amount</div>
            <Input type="number" placeholder="0.0" className="w-full" />
          </div>
        </div>
        <div className="flex items-center gap-2 text-gray-400 text-xs mb-2">
          Live Collateral Ratio:
          <span className="w-32 h-2 bg-gray-800 rounded-full inline-block"></span>
          {/* Collateral ratio progress bar (placeholder) */}
        </div>
        <div className="text-gray-400 text-xs mb-2">
          Transaction Preview: What you’ll receive + health impact will be shown
        </div>
        <Button variant="default" className="w-full">Borrow</Button>
      </Card>
    </div>
  );
}
